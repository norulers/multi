...
static void __attribute__((unused)) MJXQ_send_packet(uint8_t bind)
{
  ...
  case PHOENIX:
    ...
    if (!bind)
    {
      packet[14] = 0x04 | GET_FLAG(CH5_SW, 0x01) //FLIP
  ...
  case X800:
  default:
    packet[10] = 0x10 | GET_FLAG(!CH6_SW, 0x02) //LED
  ...
  for (uint8_t i = 1; i < MJXQ_PACKET_SIZE - 1; i++)
    sum += packet[i];
  packet[15] = sum;
...
static void __attribute__((unused)) MJXQ_init()
{
  ...
  else if (sub_protocol == H26D || sub_protocol == H26WH || sub_protocol == E010 || sub_protocol == PHOENIX)
