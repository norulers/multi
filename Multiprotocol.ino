...
// Setup diagnostic uart before anything else
  ...
  while (!Serial)
    ; // Wait for ever for the serial port to connect...
...
// reset telemetry
    ...
    case PROTO_HUBSAN:
      ...
      if (IS_BIND_BUTTON_FLAG_on)
        random_id(EEPROM_ID_OFFSET, true); // Generate new ID if bind button is pressed.
    ...
    case PROTO_DEVO:
      ...
      if (mode_select) //PPM mode
      {
        ...
        else
        {
          ...
          if (option != 1)
            option = 0; // if not fixed id mode then it should be autobind
        }
      }
#endif        //ENABLE_PPM
      PE2_on; //antenna RF4
...
{ // Interrupt on PPM pin
  ...
  { //start of frame
    if (chan >= MIN_PPM_CHANNELS)
    {
      ...
      if (chan > PPM_chan_max)
        PPM_chan_max = chan; // Saving the number of channels received
    }
