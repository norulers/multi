...
static uint16_t __attribute__((unused)) WFLY_send_data_packet()
{
  ...
  { // Send sticks packet
    ...
    if (nbr_ch < 4)
      nbr_ch = 9; // 4 channels min can be sent, default to 9
    if (nbr_ch > 9)
      nbr_ch = 9;                  // 9 channels max can be sent
...
uint16_t initWFLY()
{
  ...
  rf_ch_num = 0x15      // RF channel to send the current hopping table
      ...
      debug("ID:") for (uint8_t i = 0; i < 2; i++)
  ...
  debug("RF Channels:") for (uint8_t i = 0; i < WFLY_NUM_FREQUENCE; i++)
