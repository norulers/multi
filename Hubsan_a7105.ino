...
static void __attribute__((unused)) hubsan_build_packet()
{
  ...
  { // H107/L/C/D, H102D
    ...
    else
    {
      ...
      // Channel 6
      if (CH6_SW)
        packet[9] |= HUBSAN_FLAG_LED;
      // Channel 8
      if (CH8_SW)
        packet[9] |= HUBSAN_FLAG_VIDEO; // H102D
    }
    packet[10] = 0x64;
    //const uint32_t txid = 0xdb042679;
    packet[11] = 0xDB;
  ...
  { // H301
    ...
    {
      packet[9] = GET_FLAG(CH6_SW, FLAG_H301_LED) | GET_FLAG(CH7_SW, FLAG_H301_STAB) | GET_FLAG(CH8_SW, FLAG_H301_VIDEO) | GET_FLAG(CH5_SW, FLAG_H301_RTH);
    }
    packet[10] = 0x18; // ?
    packet[12] = 0x5c; // ?
    packet[14] = 0xf6; // ?
  ...
  { //ID_PLUS && H501
    ...
    {                                                                                                                             // H501S
      packet[9] = 0x02 | GET_FLAG(CH6_SW, FLAG_H501_LED) | GET_FLAG(CH8_SW, FLAG_H501_VIDEO) | GET_FLAG(CH12_SW, FLAG_H122D_FLIP) // H122D specific -> flip
                  | GET_FLAG(CH5_SW, FLAG_H501_RTH) | GET_FLAG(CH10_SW, FLAG_H501_GPS_HOLD) | GET_FLAG(CH9_SW, FLAG_H501_HEADLESS1);
      //packet[10]= 0x1A;
      ...
      packet[13] = GET_FLAG(CH9_SW, FLAG_H501_HEADLESS2) | GET_FLAG(CH11_SW, FLAG_H501_ALT_HOLD) | GET_FLAG(CH7_SW, FLAG_H501_SNAPSHOT);
    }
    else
    { // H107P/C+/D+
      packet[9] = 0x06;
      ...
      if (CH9_SW)
        packet[9] |= HUBSAN_FLAG_HEADLESS;
      packet[10] = 0x19;
      packet[12] = 0x5C; // ghost channel ?
      packet[13] = 0;
      if (CH7_SW)
        packet[13] = HUBSAN_FLAG_SNAPSHOT;
      if (CH5_SW)
        packet[13] |= HUBSAN_FLAG_FLIP_PLUS;
