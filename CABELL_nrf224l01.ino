...
static void __attribute__((unused)) CABELL_send_packet(uint8_t bindMode)
{
  ...
  for (int x = 0; (x < CABELL_NUM_CHANNELS - channelReduction); x++)
  {
    ...
    if (bindMode)
    {
      ...
      case THROTTLE:
        holdValue = 1000;
        break; // always set throttle to off when binding for safety
      //tx address sent for bind
      case 11:
        holdValue = 1000 + rx_tx_addr[0];
        break;
      case 12:
        holdValue = 1000 + rx_tx_addr[1];
        break;
      case 13:
        holdValue = 1000 + rx_tx_addr[2];
        break;
      case 14:
        holdValue = 1000 + rx_tx_addr[3];
        break;
      case 15:
        holdValue = 1000 + rx_tx_addr[4];
        break;
      }
    }
